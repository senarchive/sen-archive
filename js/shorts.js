...
function shToggleSound() {
    if (!shPlayer || typeof shPlayer.isMuted !== 'function') return;
    if (shPlayer.isMuted()) shPlayer.unMute();
    else shPlayer.mute();
    shUpdateSoundBtn();
}

function shUpdateSoundBtn() {
    const btn = document.getElementById('shSoundBtn');
    if (!btn || !shPlayer || typeof shPlayer.isMuted !== 'function') return;
    const muted = shPlayer.isMuted();
